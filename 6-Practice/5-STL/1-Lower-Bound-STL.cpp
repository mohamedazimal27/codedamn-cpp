...
If so, you have to tell at which index it is present and if it is not present, you have to tell the index at which
the smallest integer that is just greater than the given number is present.
...
The first line of the input contains the number of integers . The next line contains  integers in sorted order. The next line contains ,
the number of queries. Then  lines follow each containing a single integer .
...
int main() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        vector<int>::iterator it = lower_bound(v.begin(), v.end(), qs);

        if (*it == qs) {
            int index = distance(v.begin(), it) + 1; // 1-based index
            cout << "Yes " << index << endl;
        } else {
            int index = distance(v.begin(), it) + 1; // 1-based index of the next smallest number
            cout << "No " << index << endl;
        }
    }
